...
//作成者：　AT12D_35_柳沢帆貴
//-----------------------------------------------------
//2020/07/23	学校から配布されたSound.cpp&.hをクラス化
//				パンニングやフェードを実装予定
//2020/07/24	InOutのチャンネル数を返す関数LoadIOCh作成
//2020/07/25	様々なパンニングができるPan関数作成
//2020/07/29	フェード処理のプロトタイプ作成
//2020/09/03    Update追加,PlaySoundでフェードの入力可能に変更
//              InitSoundとUninitSoundを静的メンバ関数に変更
//              などを行い使いやすくした
//
/////////////////////////////////////////////////////////
...
bool CSound::Fade(float TargetVolume, float TargetTime)	//volは１か０が好ましい
{
...
////////////////////////////////////////////
//
//音声ファイルの入出力チャンネル数を取得する
//
////////////////////////////////////////////
std::tuple<int, int> CSound::LoadIOCh(IXAudio2SourceVoice* pSourceVoice, IXAudio2MasteringVoice* pMasteringVoice)
{
	XAUDIO2_VOICE_DETAILS details;
	pSourceVoice->GetVoiceDetails(&details); // ソースボイスの情報を得る
	int InChannels = details.InputChannels;
	pMasteringVoice->GetVoiceDetails(&details); // マスターボイスの情報を得る
	int OutChannels = details.InputChannels;

	return std::forward_as_tuple(InChannels, OutChannels);	//std::tie(a,b...)=LoadIOChで受け取る。a,bは任意の変数
}															//std::tie(a, std::ignore, c)でignoreの場所は受け取らない。
...
void CSound::Pan(IXAudio2SourceVoice* pSourceVoice, IXAudio2MasteringVoice* pMasteringVoice, int Type, FLOAT2 pos)//FLOAT2 = nullptr
{
	...
	case 2://近づくと音が鳴る
		m_DistanceY = (fabsf(pos.y - SCREEN_HEIGHT) / (float)SCREEN_HEIGHT);	//近いほど値は小さい
		m_DistanceX = (fabsf(pos.x - SCREEN_WIDTH * 0.5f) / (float)SCREEN_WIDTH);//スクリーンをオブジェクトのposに変える
		m_DistanceX *= 2.0f;
		...
		{
			m_Volumes[0] = 1.0f - m_DistanceX - m_DistanceY;// 左ボリューム
			m_Volumes[1] = 1.0f - m_DistanceX - m_DistanceY; // 右ボリューム
		}
